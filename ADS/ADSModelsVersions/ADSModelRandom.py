...
class ADSModelRandom(ADSModelAbstract):
    ...
    def create_model_layers(self, size_image, number_features):
        ...
        if has_dense_features:
            dense_features = final_layer_features = tf.keras.layers.Dense(random.choice([16, 32, 64, 128]))(
                input_features)
        ...
        if has_dense_resized_images:
            input_full_dense_images = final_layer_resized_images = tf.keras.layers.Dense(
                random.choice([16, 32, 64, 128]))(input_full_flatten_images)
        ...
        if has_dense_object_images:
            input_objects_dense_images = final_layer_object_images = tf.keras.layers.Dense(
                random.choice([16, 32, 64, 128]))(input_objects_flatten_images)
        ...
        input_surfaces_flatten_images = final_layer_surface_images = tf.keras.layers.Flatten()(
            final_layer_surface_images)
        if has_dense_surface_images:
            input_surfaces_dense_images = final_layer_surface_images = tf.keras.layers.Dense(
                random.choice([16, 32, 64, 128]))(input_surfaces_flatten_images)
